...
from py_vapid import Vapid, b64urldecode
...
class DeploymentCommandTests(TestCase):
    def test_generate_vapid_keys_outputs_environment_values(self):
        stdout = StringIO()
        stderr = StringIO()

        call_command("generate_vapid_keys", stdout=stdout, stderr=stderr)
        ...
        private_key = values["WEBPUSH_VAPID_PRIVATE_KEY"]
        public_key = values["WEBPUSH_VAPID_PUBLIC_KEY"]
        self.assertEqual(len(b64urldecode(public_key.encode())), 65)
        self.assertIsNotNone(Vapid.from_string(private_key).private_key)
        self.assertIn("GitHub에 올리지 마세요", stderr.getvalue())
...
class CollaborationTests(TestCase):
    ...
    def test_landing_and_login_flow(self):
        ...
        self.assertContains(landing_response, "무료로 시작하기")
    ...
    def test_shared_progress_and_peer_reminder(self, mocked_webpush):
        ...
        self.assertNotContains(page, "미완료 구성원 보기")
        self.assertContains(page, "구성원 현황")
        self.assertContains(page, '<details class="member-progress-panel">')
