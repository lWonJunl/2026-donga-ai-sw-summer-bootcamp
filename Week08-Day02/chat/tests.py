...
class RAGSecurityTests(TestCase):
    ...
    def test_pdf_text_is_normalized_and_sectioned_for_rag(self):
        from .rag_loaders import structure_pdf_text

        structured = structure_pdf_text(
            "1장 신청\n1 사업 신청은 어떻게 해야 하나요?\n☞ K-Startup에서 신청합니다.\n* (참고) 회원가입이 필요합니다.",
            "AI 공급기업 주요 질의(FAQ)",
        )

        self.assertIn("문서 제목: AI 공급기업 주요 질의(FAQ)", structured)
        self.assertIn("질문: 1 사업 신청은 어떻게 해야 하나요?", structured)
        self.assertIn("답변: K-Startup에서 신청합니다.", structured)
        self.assertIn("기타 참고사항: (참고) 회원가입이 필요합니다.", structured)

    def test_compact_pdf_text_uses_question_answer_and_note_markers(self):
        from .rag_loaders import structure_compact_faq_entries

        entries = structure_compact_faq_entries(
            "[FAQ]1 신청은 어떻게 하나요?☞ K-Startup에서 신청합니다. * 회원가입이 필요합니다. 2 중복 신청이 가능한가요?☞ 불가합니다.",
            "FAQ",
        )

        self.assertEqual(len(entries), 2)
        self.assertIn("질문: 1 신청은 어떻게 하나요?", entries[0])
        self.assertIn("답변: K-Startup에서 신청합니다.", entries[0])
        self.assertIn("기타 참고사항: 회원가입이 필요합니다.", entries[0])
        self.assertIn("질문: 2 중복 신청이 가능한가요?", entries[1])

    def test_compact_pdf_does_not_treat_counts_as_question_numbers(self):
        from .rag_loaders import structure_compact_faq_entries

        entries = structure_compact_faq_entries(
            "17\uc5ec\ub7ec\uac1c\uc758 \uc194\ub8e8\uc158\uc744 \uc2e0\uccad\ud588\uc744 \ub54c, 1\uac1c\ub9cc \uc120\uc815\ub420 \uc218\ub3c4 \uc788\ub098\uc694?"
            "\u261e \ub9de\uc2b5\ub2c8\ub2e4. 1\uac1c \uc774\uc0c1(\ucd5c\ub300 3\uac1c)\uc758 \uc0c1\ud488\uc744 \uc2e0\uccad\ud55c \uacbd\uc6b0 \uc77c\ubd80\ub9cc \uc120\uc815\ub420 \uc218 \uc788\uc2b5\ub2c8\ub2e4."
            "18\ucc3d\uc5c5\ud55c \uc9c0 \uc5bc\ub9c8 \ub418\uc9c0 \uc54a\uc544 \uc2e0\uccad \uac00\ub2a5\ud55c\uac00\uc694?"
            "\u261e \uac00\ub2a5\ud569\ub2c8\ub2e4.",
            "FAQ",
        )

        self.assertEqual(len(entries), 2)
        self.assertIn("\uc9c8\ubb38: 17\uc5ec\ub7ec\uac1c", entries[0])
        self.assertNotIn("18\ucc3d\uc5c5\ud55c", entries[0])
        self.assertIn("\uc9c8\ubb38: 18\ucc3d\uc5c5\ud55c", entries[1])

    def test_compact_pdf_removes_page_heading_from_first_question(self):
        from .rag_loaders import structure_compact_faq_entries

        entries = structure_compact_faq_entries(
            "2\uc7a5 \uc194\ub8e8\uc158 \uc81c\uacf5\u00b7\uc815\uc0b0"
            "20\ub3c4\uc804\uc790 \ub9e4\uce6d\uc744 \uc704\ud55c \uc790\uccb4 \ud64d\ubcf4\uac00 \uac00\ub2a5\ud55c\uac00\uc694?"
            "\u261e \ubd88\uac00\ud569\ub2c8\ub2e4."
            "21\uc120\uc815 \ud6c4 \uc194\ub8e8\uc158\uc744 \ucd94\uac00 \ub4f1\ub85d\ud560 \uc218 \uc788\ub098\uc694?"
            "\u261e \ubd88\uac00\ud569\ub2c8\ub2e4.",
            "FAQ",
        )

        self.assertEqual(len(entries), 2)
        self.assertIn("\uc9c8\ubb38: 20\ub3c4\uc804\uc790", entries[0])
        self.assertNotIn("2\uc7a5", entries[0])
        self.assertIn("\uc9c8\ubb38: 21\uc120\uc815", entries[1])
    ...
    @patch("chat.rag_store.get_vector_store")
    def test_missing_milvus_collection_is_treated_as_no_vectors(self, mocked_store):
        from .rag_store import delete_source_documents

        mocked_store.return_value.delete.return_value = False
        mocked_store.return_value.client.has_collection.return_value = False

        delete_source_documents(1, "https://example.com/missing-collection")
