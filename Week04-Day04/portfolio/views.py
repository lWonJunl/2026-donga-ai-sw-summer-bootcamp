...
from .forms import ProfileForm, SecureSignupForm
...
PROJECT_ARCHIVE = [
    ...
    },
    {
        ...
        "link": "https://github.com/lWonJunl/2026-donga-ai-sw-summer-bootcamp/blob/main/Week01-Day02/README_Google_News_RSS.md",
    },
    {
        "year": "2026",
        "category": "Algorithm",
        ...
        "link": "https://github.com/lWonJunl/2026-donga-ai-sw-summer-bootcamp/blob/main/Week01-Day04/README_Image_Maze_Solver.md",
    },
    {
        "year": "2023",
        "category": "Machine Learning",
    ...
    },
    {
        ...
        "link": "https://github.com/lWonJunl/2026-donga-ai-sw-summer-bootcamp/blob/main/Week01-Day03/README_IDA_Star_Rubiks_Cube.md",
    },
    {
        "year": "2023",
        "category": "Python",
...
def _read_api_projects():
    data_path = Path(settings.BASE_DIR) / "data" / "projects.txt"
        ...
        projects = [
            {
                ...
                "technologies": [value.strip() for value in row["technologies"].split(",") if value.strip()],
            }
            for row in rows
        ]
...
def _api_project_language(project, language):
...
def api_login_required(view_func):
    """Require a Django-authenticated session and keep API errors as JSON."""
    @wraps(view_func)
    def wrapped_view(request, *args, **kwargs):
        if not request.user.is_authenticated:
            if "text/html" in request.headers.get("Accept", ""):
                ...
                response = redirect(f"{reverse('login')}?{query}")
            else:
...
def api_health(request):
    try:
        data_path, projects = _read_api_projects()
    except (OSError, KeyError, TypeError, ValueError):
        logger.exception("Unable to read API project data")
        return JsonResponse({"success": False, "status": "error"}, status=500)
    return JsonResponse({"success": True, "status": "ok", "source": str(data_path.relative_to(settings.BASE_DIR)), "count": len(projects)})
...
def api_projects(request):
    try:
        _, projects = _read_api_projects()
    except (OSError, KeyError, TypeError, ValueError):
        logger.exception("Unable to read API project data")
        return JsonResponse({"success": False, "error": "Internal server error"}, status=500)

    category = request.GET.get("category", "").strip()
    keyword = request.GET.get("q", "").strip().casefold()
    language = request.GET.get("lang", "all")
    language = language if language in {"ko", "en"} else "all"
    categories = sorted({project["category"] for project in projects})
    if category:
        projects = [project for project in projects if project["category"].casefold() == category.casefold()]
    if keyword:
        projects = [
            project for project in projects
            if keyword in " ".join([
                project["title"]["ko"], project["title"]["en"],
                project["description"]["ko"], project["description"]["en"],
                *project["technologies"],
            ]).casefold()
        ]
    return JsonResponse(
        {"success": True, "count": len(projects), "categories": categories, "data": [_api_project_language(project, language) for project in projects]},
...
def api_project_detail(request, project_id):
    ...
    language = request.GET.get("lang", "all")
    language = language if language in {"ko", "en"} else "all"
    return JsonResponse({"success": True, "data": _api_project_language(project, language)}, json_dumps_params={"ensure_ascii": False})
...
def api_portfolio(request):
    ...
    archive = localize_items(PROJECT_ARCHIVE, ARCHIVE_TRANSLATIONS, language)
    for project in archive:
        project["category_label"] = project["category"]
    data = {
        "profile": profile,
        "social_links": deepcopy(SOCIAL_LINKS),
        "stats": deepcopy(STATS),
        "skills": deepcopy(SKILLS),
        "featured_projects": localize_items(PROJECTS, PROJECT_TRANSLATIONS, language),
    ...
    return JsonResponse({"success": True, "language": language, "data": data}, json_dumps_params={"ensure_ascii": False})
...
def _safe_next_url(request, default="api_page"):
...
def _rate_key(scope, request, identity=""):
    raw = f"{scope}:{request.META.get('REMOTE_ADDR', 'unknown')}:{identity.casefold()}"
    return f"auth-rate:{hashlib.sha256(raw.encode()).hexdigest()}"
...
def register_view(request):
    ...
    if request.method == "GET" and request.GET.get("next") and request.GET.get("intent") != "register":
        query = urlencode(
            {
    ...
    form = SecureSignupForm(request.POST or None)
    status = 200
    if request.method == "POST":
        rate_key = _rate_key("register", request)
        if cache.get(rate_key, 0) >= 10:
            form.add_error(None, "가입 요청이 너무 많습니다. 잠시 후 다시 시도해 주세요.")
            status = 429
        elif form.is_valid():
            ...
            _record_attempt(rate_key, timeout=3600)
    return render(request, "auth/register.html", {"form": form, "next": _safe_next_url(request)}, status=status)
...
def login_view(request):
    ...
    status = 200
    if request.method == "POST":
        rate_key = _rate_key("login", request, request.POST.get("username", ""))
        if cache.get(rate_key, 0) >= 5:
            form.add_error(None, "로그인 시도가 너무 많습니다. 5분 후 다시 시도해 주세요.")
            status = 429
        elif form.is_valid():
            user = form.get_user()
            if not EmailAddress.objects.filter(user=user, verified=True).exists():
                form.add_error(None, "이메일 인증을 완료한 후 로그인해 주세요.")
            else:
                cache.delete(rate_key)
                login(request, user)
                return redirect(_safe_next_url(request))
        else:
            _record_attempt(rate_key, timeout=300)
    provider_labels = {
        "google": "Google",
        "github": "GitHub",
        "kakao": "Kakao",
    ...
    for provider_id, label in provider_labels.items():
        app = settings.SOCIALACCOUNT_PROVIDERS[provider_id]["APP"]
    ...
    return render(
        request,
        "auth/login.html",
        {"form": form, "next": _safe_next_url(request), "social_providers": social_providers},
        status=status,
    )
...
def profile_view(request):
    ...
    email_addresses = EmailAddress.objects.filter(user=request.user).order_by("-primary", "email")
    return render(
        request,
        "auth/profile.html",
        {"form": form, "email_addresses": email_addresses},
    )
