...
- **의도:** 주변 친구들의 공동 입력에 남은 시간과 개인 진행 상태 분석을 더하고, 마감일이 가까워질수록 알림 빈도를 높여 과제를 잊지 않도록 돕습니다.
...
- 학년도·학기·과목명·분반을 필수로 입력해 그룹 생성
- 담당 교수와 그룹 별칭 선택 입력
- 무작위 8자리 초대 코드로 그룹 참여
- 참여 중인 과목을 카드형 또는 목록형으로 전환하고 선택 상태를 브라우저에 저장
- 그룹 정보 수정과 과목명 확인을 거친 그룹 삭제
- 관리자 지정·해제와 구성원 내보내기
...
| `ClassGroup` | 학년도·학기·과목명·분반·초대 코드 |
...
현재 테스트는 위험도, 인증, 로그인 잠금, 요청 제한, 그룹 권한, 과제 권한, 개인 진행 상태, 회원 탈퇴, 푸시 구독, 자동 알림 시각과 중복 방지를 검사합니다.
