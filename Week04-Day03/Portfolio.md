...
Vercel은 저장소 루트의 `manage.py`를 감지해 Django 애플리케이션과 정적 파일을 자동으로 빌드합니다.

### 1. Deployment dependencies

`requirements.txt`에는 Django와 WhiteNoise만 유지합니다. Vercel의 Python Runtime이 WSGI 애플리케이션을 실행하므로 Gunicorn은 필요하지 않습니다.
...
`.python-version`에는 지원되는 Python 버전을 지정합니다.
...
3. Root Directory를 `Week04-Day03`으로 지정합니다.
4. Framework Preset, Build Command와 Output Directory는 자동 감지 또는 기본값으로 둡니다.
5. Production과 Preview 환경에 `DJANGO_SECRET_KEY`와 `DJANGO_DEBUG=False`를 추가합니다.
6. 배포 후 Vercel이 제공한 `https://<project-name>.vercel.app` 주소를 확인합니다.

`portfolio/settings.py`는 `VERCEL_URL`과 `VERCEL_PROJECT_PRODUCTION_URL`을 허용 호스트와 CSRF 신뢰 출처에 자동으로 반영합니다. 비밀키는 GitHub에 올리지 않고 Vercel의 Environment Variables에서만 관리합니다.

### 3. Local verification

배포 전에 Django 설정과 정적 파일 수집을 확인합니다.
...
| [`requirements.txt`](requirements.txt) | Vercel 빌드에 필요한 Django와 WhiteNoise 의존성 |
| [`.python-version`](.python-version) | Vercel Python Runtime 버전 지정 |
